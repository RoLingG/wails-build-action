# Build Options (sub-action)

Purpose
- Computes a `BUILD_OPTIONS` string for `wails build` based on inputs and the detected Ubuntu distro.
...
- `distro` (optional) — Pass the distro output from linux-discovery; enables 24.04 special case.
...
- name: Compute Build Options
  id: opts
  uses: snider/build-action/actions/build-options@v3
  with:
